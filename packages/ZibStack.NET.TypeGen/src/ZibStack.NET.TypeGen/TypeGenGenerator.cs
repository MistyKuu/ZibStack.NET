...
#pragma warning disable RS1035 // File I/O is intentional here — see method summary.
    private static void TryWriteFilesDirectly(IReadOnlyList<EmittedFile> files, string projectDir, SourceProductionContext spc)
    {
        ...
        // Also sweep parent directories of each touchedDir — catches stale files
        // from previous OutputDir values that share a common ancestor. For example,
        // if OutputDir changed from "generated/ts" to "generated/client", the parent
        // "generated/" is the same, so we also sweep "generated/ts/".
        var parentDirs = new HashSet<string>(System.StringComparer.OrdinalIgnoreCase);
        foreach (var td in touchedDirs)
        {
            try
            {
                var parent = System.IO.Directory.GetParent(td)?.FullName;
                if (parent != null && !string.Equals(System.IO.Path.GetFullPath(parent),
                    System.IO.Path.GetFullPath(projectDir), System.StringComparison.OrdinalIgnoreCase))
        ...
        foreach (var parent in parentDirs)
        {
            try
            {
                foreach (var sibling in System.IO.Directory.EnumerateDirectories(parent))
                {
                    var fullSibling = System.IO.Path.GetFullPath(sibling);
                    if (touchedDirs.Contains(fullSibling)) continue;
                    // Check if sibling contains any file with our banner.
                    foreach (var candidate in System.IO.Directory.EnumerateFiles(sibling))
                    {
                        try
                        {
                            using var fs = new System.IO.FileStream(candidate,
                                System.IO.FileMode.Open, System.IO.FileAccess.Read, System.IO.FileShare.Read);
                            var buf = new byte[200];
                            int n = fs.Read(buf, 0, buf.Length);
                            var head = System.Text.Encoding.UTF8.GetString(buf, 0, n);
                            if (head.IndexOf("@generated by ZibStack.NET.TypeGen", System.StringComparison.Ordinal) >= 0)
                            {
                                touchedDirs.Add(fullSibling);
                                break;
                            }
                        }
                        catch { }
            ...
            catch { /* parent enumeration failed — rely on MSBuild task sweep */ }
        }
