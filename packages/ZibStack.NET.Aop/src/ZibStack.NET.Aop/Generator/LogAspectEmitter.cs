...
{
    public string AttributeFullName => "ZibStack.NET.Aop.LogAttribute";
    ...
    private static string FormatReturnValue(InterceptedMethodModel method, AspectInfo aspect, int objectLogging)
    {
        ...
        // `?? "null"` keeps the arg non-null so the `Action<ILogger, …, object, Exception?>`
        // delegate signature generated by `LoggerMessage.Define` doesn't trip CS8604.
        return "(((object?)__result)?.ToString() ?? \"null\")";
    }
    ...
    {
        if (cls.AspectClassData.TryGetValue("ZibStack.NET.Aop.LogAttribute", out var d) && d.TryGetValue(key, out var v)) return v;
        return null;
    }
...
internal sealed class LogClassDataProvider : IClassDataProvider
{
    ...
    public IReadOnlyDictionary<string, object?>? ExtractClassData(INamedTypeSymbol classSymbol)
    {
        ...
        bool hasLog = classSymbol.GetMembers().OfType<IMethodSymbol>()
            .Any(m => m.GetAttributes().Any(a => a.AttributeClass?.ToDisplayString() == "ZibStack.NET.Aop.LogAttribute"));
    ...
    public IEnumerable<Diagnostic> GetDiagnostics(INamedTypeSymbol classSymbol)
    {
        ...
        foreach (var member in classSymbol.GetMembers())
        {
            if (member is IMethodSymbol method && method.IsStatic &&
                method.GetAttributes().Any(a => a.AttributeClass?.ToDisplayString() == "ZibStack.NET.Aop.LogAttribute"))
            {
                ...
                if (loc != null)
                    yield return Diagnostic.Create(LogDiagnosticDescriptors.StaticMethodNotSupported, loc, method.Name);
            }
        }
    }
}
