...
public partial class DtoGenerator
{
    ...
    private static string GenerateCrudTestSource(CrudTestInfo info)
    {
        ...
        var createBody = BuildAnonymousObject(createProps, forUpdate: false);
        var updateBody = BuildAnonymousObject(updateProps, forUpdate: true);
            ...
            (info.Operations & OpUpdate) != 0 && (info.Operations & OpDelete) != 0)
        {
            ...
            // Verify response body fields match what we sent
            if (createProps.Count > 0)
            {
                sb.AppendLine("        var body = await getResponse.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();");
                foreach (var p in createProps.Take(3))
                {
                    var jsonName = char.ToLowerInvariant(p.Name[0]) + p.Name.Substring(1);
                    var (_, assertValue) = TestValue(p.CSharpType, p.Name, false);
                    if (p.CSharpType == "string" || p.CSharpType == "System.String")
                        sb.AppendLine($"        Assert.Equal({assertValue}, body.GetProperty(\"{jsonName}\").GetString());");
                    else if (p.CSharpType is "int" or "System.Int32")
                        sb.AppendLine($"        Assert.Equal({assertValue}, body.GetProperty(\"{jsonName}\").GetInt32());");
                    else if (p.CSharpType is "decimal" or "System.Decimal" or "double" or "System.Double" or "float" or "System.Single")
                        sb.AppendLine($"        Assert.True(body.GetProperty(\"{jsonName}\").GetDecimal() > 0);");
                }
            }
    ...
    private static string BuildAnonymousObject(List<CrudTestPropertyInfo> props, bool forUpdate)
    {
    ...
    private static (string literal, string assertValue) TestValue(string csharpType, string propName, bool forUpdate)
    {
        ...
        return csharpType switch
        {
            ...
            "double" or "System.Double" => (forUpdate ? "199.99" : "99.99", forUpdate ? "199.99" : "99.99"),
            "float" or "System.Single" => (forUpdate ? "199.99f" : "99.99f", forUpdate ? "199.99f" : "99.99f"),
            "bool" or "System.Boolean" => ("true", "true"),
            "System.DateTime" => ("System.DateTime.UtcNow", "System.DateTime.UtcNow"),
            "System.Guid" => ("System.Guid.NewGuid()", "System.Guid.NewGuid()"),
            _ => ("default", "default"),
        };
    }
