...
public partial class DtoGenerator
{
    ...
    private static string GenerateMinimalApiSource(CrudApiInfo info)
    {
        ...
        // [ColumnPermission] enforcement: endpoints mask restricted response columns automatically.
        var enforcePerms = info.ColumnPermissions.Count > 0 && info.HasResponseDto && info.ResponseName is not null;
        var permsClass = $"{entity}ColumnPermissions";
        const string userParamStr = ", System.Security.Claims.ClaimsPrincipal? __user";
        var userParam = enforcePerms ? userParamStr : "";
        // The list endpoint only needs masking when a restricted column survives in the list DTO.
        var enforceListPerms = enforcePerms && (info.ListResponseName is null || info.ListColumnPermissions.Count > 0);

        // Optimistic concurrency: ETag response header + If-Match precondition checks.
        var concurrency = info.Concurrency;
        var httpParam = concurrency ? ", HttpContext __http" : "";
        var ifMatchParam = concurrency ? ", [Microsoft.AspNetCore.Mvc.FromHeader(Name = \"If-Match\")] string? ifMatch = null" : "";

        // Audit fields: write endpoints stamp CreatedAt/UpdatedAt + CreatedBy/UpdatedBy.
        var audit = info.Audit;
        var writeUserParam = enforcePerms || audit ? userParamStr : "";
        var deleteUserParam = audit && info.SoftDelete ? userParamStr : "";
        ...
        {
            sb.AppendLine($"        group.MapGet(\"{{id}}\", async ({keyType} id, {storeType} store{userParam}{httpParam}, CancellationToken ct) =>");
            sb.AppendLine("        {");
            ...
            if (concurrency)
                sb.AppendLine("            __http.Response.Headers.ETag = ZibStack.NET.Dto.ETag.Format(entity.RowVersion);");
            if (info.HasResponseDto && info.ResponseName is not null)
            {
                ...
                sb.AppendLine(enforcePerms
                    ? $"            return Results.Ok({permsClass}.Apply({fqResponse}.FromEntity(entity), __user));"
                    : $"            return Results.Ok({fqResponse}.FromEntity(entity));");
            }
            else
            {
        ...
        if ((info.Operations & OpGetList) != 0)
        {
            ...
            if (info.HasQueryDsl && fqQuery is not null)
            {
                ...
                if (enforcePerms)
                    sb.AppendLine($"                {permsClass}.FilterFields(fields, __user);");
        ...
        if ((info.Operations & OpCreate) != 0 && info.CreateRequestName is not null)
        {
            ...
            sb.AppendLine($"        group.MapPost(\"\", async ({fqCreate} request, {storeType} store{hubParam}{writeUserParam}{httpParam}, CancellationToken ct) =>");
            sb.AppendLine("        {");
            ...
            if (audit)
            {
                ...
                sb.AppendLine("            entity.CreatedBy = __user?.Identity?.Name;");
                sb.AppendLine("            entity.UpdatedBy = __user?.Identity?.Name;");
            }
            ...
            {
                var fqResponse = info.Namespace is not null ? $"{info.Namespace}.{info.ResponseName}" : info.ResponseName;
                var createdValue = enforcePerms
                    ? $"{permsClass}.Apply({fqResponse}.FromEntity(entity), __user)"
                    : $"{fqResponse}.FromEntity(entity)";
                sb.AppendLine($"            return Results.CreatedAtRoute(\"Get{entity}\", new {{ id = entity.{info.KeyPropertyName} }}, {createdValue});");
            }
            else
            {
        ...
        if ((info.Operations & OpUpdate) != 0 && info.UpdateRequestName is not null)
        {
            ...
            sb.AppendLine($"        group.MapPatch(\"{{id}}\", async ({keyType} id, {fqUpdate} request, {storeType} store{hubParam}{writeUserParam}{httpParam}{ifMatchParam}, CancellationToken ct = default) =>");
            sb.AppendLine("        {");
            sb.AppendLine("            var entity = await store.GetByIdAsync(id, ct);");
            sb.AppendLine("            if (entity is null) return Results.Problem(statusCode: 404, title: \"Not Found\");");
            if (concurrency)
            {
                sb.AppendLine("            if (ifMatch is null) return Results.Problem(statusCode: 428, title: \"Precondition Required\", detail: \"Send If-Match with the ETag from GET (or If-Match: *).\");");
                sb.AppendLine("            if (!ZibStack.NET.Dto.ETag.Matches(ifMatch, entity.RowVersion)) return Results.Problem(statusCode: 412, title: \"Precondition Failed\", detail: \"The resource was modified by someone else. GET it again and retry with the fresh ETag.\");");
            }
            sb.AppendLine($"            var validation = request.{validateMethod}();");
            sb.AppendLine("            if (!validation.IsValid) return Results.ValidationProblem(validation.ToDictionary());");
            ...
            if (concurrency)
                sb.AppendLine("            entity.RowVersion++;");
        ...
        {
            sb.AppendLine($"        group.MapDelete(\"{{id}}\", async ({keyType} id, {storeType} store{hubParam}{deleteUserParam}{ifMatchParam}, CancellationToken ct = default) =>");
            sb.AppendLine("        {");
            sb.AppendLine("            var entity = await store.GetByIdAsync(id, ct);");
            sb.AppendLine("            if (entity is null) return Results.Problem(statusCode: 404, title: \"Not Found\");");
            if (concurrency)
                sb.AppendLine("            if (ifMatch is not null && !ZibStack.NET.Dto.ETag.Matches(ifMatch, entity.RowVersion)) return Results.Problem(statusCode: 412, title: \"Precondition Failed\", detail: \"The resource was modified by someone else. GET it again and retry with the fresh ETag.\");");
            if (info.SoftDelete)
            {
                sb.AppendLine("            if (entity.IsDeleted) return Results.Problem(statusCode: 404, title: \"Not Found\");");
        ...
        if ((info.Operations & OpBulkCreate) != 0 && info.CreateRequestName is not null)
        {
            var fqCreate = info.Namespace is not null ? $"{info.Namespace}.{info.CreateRequestName}" : info.CreateRequestName;
            var validateMethod = info.IsCombinedDto ? "ValidateForCreate" : "Validate";
            sb.AppendLine($"        group.MapPost(\"bulk\", async (List<{fqCreate}> requests, {storeType} store{writeUserParam}, CancellationToken ct) =>");
            sb.AppendLine("        {");
            ...
            if (info.HasResponseDto && info.ResponseName is not null)
            {
                var fqResponse = info.Namespace is not null ? $"{info.Namespace}.{info.ResponseName}" : info.ResponseName;
                sb.AppendLine(enforcePerms
                    ? $"            return Results.Ok(entities.Select(e => {permsClass}.Apply({fqResponse}.FromEntity(e), __user)).ToList());"
                    : $"            return Results.Ok(entities.Select(e => {fqResponse}.FromEntity(e)).ToList());");
            }
            else
            {
        ...
        {
            sb.AppendLine($"        group.MapPost(\"bulk-delete\", async (List<{keyType}> ids, {storeType} store{deleteUserParam}, CancellationToken ct) =>");
            sb.AppendLine("        {");
    ...
    /// <summary>
    /// Emits a standalone <c>{Entity}ColumnPermissions</c> class so both the minimal API
    /// endpoints and the generated controller can mask restricted columns. Null/anonymous
    /// users are treated as having no permissions (safe by default).
    /// </summary>
    private static string GenerateColumnPermissionsSource(CrudApiInfo info)
    {
        var sb = new StringBuilder();
        var entity = info.ClassName;
        var fqResponse = info.Namespace is not null ? $"{info.Namespace}.{info.ResponseName}" : info.ResponseName!;
        ...
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine($"// Generated by ZibStack.NET.Dto from [ColumnPermission] on {entity}");
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine($"namespace {info.Namespace ?? "ZibStack.NET.Dto.Generated"};");
        sb.AppendLine();
        sb.AppendLine($"/// <summary>Masks <see cref=\"{fqResponse}\"/> columns the current user lacks permission for. Called automatically by the generated CRUD endpoints.</summary>");
        sb.AppendLine($"public static class {entity}ColumnPermissions");
        sb.AppendLine("{");
        sb.AppendLine("    private static bool Has(System.Security.Claims.ClaimsPrincipal? user, string permission)");
        sb.AppendLine("        => user is not null && (user.HasClaim(\"permission\", permission) || user.IsInRole(permission));");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Nulls restricted properties unless the user holds the required permission claim or role.</summary>");
        sb.AppendLine($"    public static {fqResponse} Apply({fqResponse} response, System.Security.Claims.ClaimsPrincipal? user)");
        sb.AppendLine("    {");
        foreach (var kv in info.ColumnPermissions)
        {
            sb.AppendLine($"        if (!Has(user, \"{kv.Value}\"))");
            sb.AppendLine($"            response = response with {{ {kv.Key} = default }};");
        }
        sb.AppendLine("        return response;");
        sb.AppendLine("    }");

        if (info.ListResponseName is not null && info.ListColumnPermissions.Count > 0)
        {
            var fqList = info.Namespace is not null ? $"{info.Namespace}.{info.ListResponseName}" : info.ListResponseName;
            sb.AppendLine();
            sb.AppendLine("    /// <summary>List-item overload — covers restricted columns that survive in the list DTO.</summary>");
            sb.AppendLine($"    public static {fqList} Apply({fqList} response, System.Security.Claims.ClaimsPrincipal? user)");
            sb.AppendLine("    {");
            foreach (var kv in info.ListColumnPermissions)
            {
                sb.AppendLine($"        if (!Has(user, \"{kv.Value}\"))");
                sb.AppendLine($"            response = response with {{ {kv.Key} = default }};");
            }
        ...
        sb.AppendLine();
        sb.AppendLine("    /// <summary>Removes restricted field paths from a <c>select=</c> projection for users without the required permission.</summary>");
        sb.AppendLine("    public static void FilterFields(System.Collections.Generic.HashSet<string> fields, System.Security.Claims.ClaimsPrincipal? user)");
        sb.AppendLine("    {");
        foreach (var kv in info.ColumnPermissions)
        {
            sb.AppendLine($"        if (!Has(user, \"{kv.Value}\"))");
            sb.AppendLine($"            fields.RemoveWhere(f => f.Equals(\"{kv.Key}\", System.StringComparison.OrdinalIgnoreCase) || f.StartsWith(\"{kv.Key}.\", System.StringComparison.OrdinalIgnoreCase));");
        }
        sb.AppendLine("    }");
        sb.AppendLine("}");
    ...
    private static string GenerateControllerSource(CrudApiInfo info)
    {
        ...
        // [ColumnPermission] enforcement — controllers read the principal from ControllerBase.User.
        var enforcePerms = info.ColumnPermissions.Count > 0 && info.HasResponseDto && info.ResponseName is not null;
        var permsClass = $"{entity}ColumnPermissions";
        var enforceListPerms = enforcePerms && (info.ListResponseName is null || info.ListColumnPermissions.Count > 0);

        // Optimistic concurrency — ETag via Response.Headers, If-Match via [FromHeader].
        var concurrency = info.Concurrency;
        var ifMatchParam = concurrency ? "[FromHeader(Name = \"If-Match\")] string? ifMatch, " : "";

        // Audit fields — stamped from ControllerBase.User.
        var audit = info.Audit;
        ...
        if ((info.Operations & OpGetById) != 0)
        {
            ...
            if (concurrency)
                sb.AppendLine("        Response.Headers.ETag = ZibStack.NET.Dto.ETag.Format(entity.RowVersion);");
            if (info.HasResponseDto && info.ResponseName is not null)
            {
                var fqResponse = info.Namespace is not null ? $"{info.Namespace}.{info.ResponseName}" : info.ResponseName;
                sb.AppendLine(enforcePerms
                    ? $"        return Ok({permsClass}.Apply({fqResponse}.FromEntity(entity), User));"
                    : $"        return Ok({fqResponse}.FromEntity(entity));");
            }
            else
            {
        ...
        if ((info.Operations & OpGetList) != 0)
        {
            ...
            if (fqCtrlList is not null)
            {
                ...
                if (enforceListPerms)
                    sb.AppendLine($"        result = result.Map(r => {permsClass}.Apply(r, User));");
            }
            else
            {
        ...
        if ((info.Operations & OpCreate) != 0 && info.CreateRequestName is not null)
        {
            ...
            if (audit)
            {
                sb.AppendLine("        entity.CreatedAt = System.DateTime.UtcNow;");
                sb.AppendLine("        entity.UpdatedAt = System.DateTime.UtcNow;");
                sb.AppendLine("        entity.CreatedBy = User?.Identity?.Name;");
                sb.AppendLine("        entity.UpdatedBy = User?.Identity?.Name;");
            }
            ...
            if (info.HasResponseDto && info.ResponseName is not null)
            {
                var fqResponse = info.Namespace is not null ? $"{info.Namespace}.{info.ResponseName}" : info.ResponseName;
                var createdValue = enforcePerms
                    ? $"{permsClass}.Apply({fqResponse}.FromEntity(entity), User)"
                    : $"{fqResponse}.FromEntity(entity)";
                sb.AppendLine($"        return CreatedAtAction(nameof(GetById), new {{ id = entity.{info.KeyPropertyName} }}, {createdValue});");
            }
            else
            {
        ...
        if ((info.Operations & OpUpdate) != 0 && info.UpdateRequestName is not null)
        {
            ...
            sb.AppendLine($"    public async Task<IActionResult> Update({keyType} id, [FromBody] {fqUpdate} request, {ifMatchParam}CancellationToken ct = default)");
            sb.AppendLine("    {");
            sb.AppendLine("        var entity = await _store.GetByIdAsync(id, ct);");
            sb.AppendLine("        if (entity is null) return NotFound();");
            if (concurrency)
            {
                sb.AppendLine("        if (ifMatch is null) return Problem(statusCode: 428, title: \"Precondition Required\", detail: \"Send If-Match with the ETag from GET (or If-Match: *).\");");
                sb.AppendLine("        if (!ZibStack.NET.Dto.ETag.Matches(ifMatch, entity.RowVersion)) return Problem(statusCode: 412, title: \"Precondition Failed\", detail: \"The resource was modified by someone else. GET it again and retry with the fresh ETag.\");");
            }
            sb.AppendLine($"        var validation = request.{validateMethod}();");
            sb.AppendLine("        if (!validation.IsValid) { foreach (var kv in validation.Errors) foreach (var msg in kv.Value) ModelState.AddModelError(kv.Key, msg); return ValidationProblem(); }");
            sb.AppendLine("        request.ApplyTo(entity);");
        ...
        if ((info.Operations & OpDelete) != 0)
        {
            ...
            sb.AppendLine($"    public async Task<IActionResult> Delete({keyType} id, {ifMatchParam}CancellationToken ct = default)");
            sb.AppendLine("    {");
            sb.AppendLine("        var entity = await _store.GetByIdAsync(id, ct);");
            sb.AppendLine("        if (entity is null) return NotFound();");
            if (concurrency)
                sb.AppendLine("        if (ifMatch is not null && !ZibStack.NET.Dto.ETag.Matches(ifMatch, entity.RowVersion)) return Problem(statusCode: 412, title: \"Precondition Failed\", detail: \"The resource was modified by someone else. GET it again and retry with the fresh ETag.\");");
            if (info.SoftDelete)
            {
                sb.AppendLine("        if (entity.IsDeleted) return NotFound();");
                sb.AppendLine("        entity.IsDeleted = true;");
            ...
            var info = new CrudTestInfo
            {
                ...
                foreach (var arg in attr.NamedArguments)
                {
                    switch (arg.Key)
                    {
                        ...
                        case "Concurrency": if (arg.Value.Value is true) info.Concurrency = true; break;
                        case "KeyProperty":
    ...
    private static string GenerateCrudTestSource(CrudTestInfo info)
    {
        ...
        // [CrudApi(Concurrency = true)] endpoints demand If-Match on PATCH (and honor it
        // on DELETE) — generated smoke tests bypass the version check with If-Match: *.
        string PatchCall(string urlExpr, string contentExpr) => info.Concurrency
            ? $"await SendWithIfMatchAsync(HttpMethod.Patch, {urlExpr}, {contentExpr})"
            : $"await _client.PatchAsync({urlExpr}, {contentExpr})";
        string DeleteCall(string urlExpr) => info.Concurrency
            ? $"await SendWithIfMatchAsync(HttpMethod.Delete, {urlExpr})"
            : $"await _client.DeleteAsync({urlExpr})";
        ...
        {
            sb.AppendLine();
            sb.AppendLine("    /// <summary>Concurrency-enabled endpoints require If-Match; * explicitly bypasses the version check.</summary>");
            sb.AppendLine("    private Task<HttpResponseMessage> SendWithIfMatchAsync(HttpMethod method, string url, HttpContent? content = null)");
            sb.AppendLine("    {");
            sb.AppendLine("        var req = new HttpRequestMessage(method, url) { Content = content };");
            sb.AppendLine("        req.Headers.TryAddWithoutValidation(\"If-Match\", \"*\");");
            sb.AppendLine("        return _client.SendAsync(req);");
            sb.AppendLine("    }");
        }
            ...
            (info.Operations & OpUpdate) != 0 && (info.Operations & OpDelete) != 0)
        {
            ...
            sb.AppendLine($"        var updateResponse = {PatchCall("location", "patchContent")};");
            ...
            sb.AppendLine($"        var deleteResponse = {DeleteCall("location")};");
        ...
        else if ((info.Operations & OpUpdate) != 0 && (info.Operations & OpCreate) != 0)
        {
            ...
            sb.AppendLine($"        var patchContent = JsonContent.Create({updateBody});");
            sb.AppendLine($"        var updateResponse = {PatchCall($"$\"/{route}/{{id}}\"", "patchContent")};");
            sb.AppendLine("        Assert.Equal(HttpStatusCode.OK, updateResponse.StatusCode);");
            sb.AppendLine("    }");
        }
        ...
        else if ((info.Operations & OpDelete) != 0 && (info.Operations & OpCreate) != 0)
        {
            ...
            sb.AppendLine($"        var body = await createResponse.Content.ReadFromJsonAsync<System.Text.Json.JsonElement>();");
            sb.AppendLine($"        var id = body.GetProperty(\"{keyJsonName}\").{keyGetter}();");
            sb.AppendLine($"        var deleteResponse = {DeleteCall($"$\"/{route}/{{id}}\"")};");
            sb.AppendLine("        Assert.Equal(HttpStatusCode.NoContent, deleteResponse.StatusCode);");
            sb.AppendLine("    }");
        }
    ...
    private static string GenerateAuditProperties(CrudApiInfo info)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("#nullable enable");
        ...
        sb.AppendLine("{");
        var first = true;
        foreach (var field in info.AuditFieldsToGenerate)
        {
            ...
            {
                case "CreatedAt":
                    sb.AppendLine("    /// <summary>UTC creation timestamp. Stamped by the generated POST endpoint when <c>[CrudApi(Audit = true)]</c>.</summary>");
                    sb.AppendLine("    public System.DateTime CreatedAt { get; set; }");
                    break;
                case "UpdatedAt":
                    sb.AppendLine("    /// <summary>UTC last-modified timestamp. Refreshed by the generated POST/PATCH/soft-DELETE endpoints.</summary>");
                    sb.AppendLine("    public System.DateTime UpdatedAt { get; set; }");
                    break;
                case "CreatedBy":
                    sb.AppendLine("    /// <summary>Identity name of the creator. Null for anonymous callers.</summary>");
                    sb.AppendLine("    public string? CreatedBy { get; set; }");
                    break;
                case "UpdatedBy":
                    sb.AppendLine("    /// <summary>Identity name of the last modifier. Null for anonymous callers.</summary>");
    ...
    private static string GenerateConcurrencyProperties(CrudApiInfo info)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("#nullable enable");
        ...
        sb.AppendLine($"partial class {info.ClassName}");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>Optimistic-concurrency version. Incremented by the generated PATCH/DELETE endpoints");
        sb.AppendLine("    /// when <c>[CrudApi(Concurrency = true)]</c>; surfaced to clients as a weak ETag.</summary>");
        sb.AppendLine("    public long RowVersion { get; set; }");
        sb.AppendLine("}");
