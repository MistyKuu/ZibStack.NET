...
public partial class DtoGenerator
{
    ...
    private static string GenerateMinimalApiSource(CrudApiInfo info)
    {
        ...
        var userParam = enforcePerms ? ", System.Security.Claims.ClaimsPrincipal? __user" : "";
        ...
        if ((info.Operations & OpCreate) != 0 && info.CreateRequestName is not null)
        {
            ...
            sb.AppendLine($"        group.MapPost(\"\", async ({fqCreate} request, {storeType} store{hubParam}{userParam}{httpParam}, CancellationToken ct) =>");
            sb.AppendLine("        {");
        ...
        if ((info.Operations & OpUpdate) != 0 && info.UpdateRequestName is not null)
        {
            ...
            sb.AppendLine($"        group.MapPatch(\"{{id}}\", async ({keyType} id, {fqUpdate} request, {storeType} store{hubParam}{userParam}{httpParam}{ifMatchParam}, CancellationToken ct = default) =>");
            sb.AppendLine("        {");
            sb.AppendLine("            var entity = await store.GetByIdAsync(id, ct);");
            sb.AppendLine("            if (entity is null) return Results.Problem(statusCode: 404, title: \"Not Found\");");
            if (concurrency)
        ...
        {
            sb.AppendLine($"        group.MapDelete(\"{{id}}\", async ({keyType} id, {storeType} store{hubParam}{ifMatchParam}, CancellationToken ct = default) =>");
            sb.AppendLine("        {");
            sb.AppendLine("            var entity = await store.GetByIdAsync(id, ct);");
            sb.AppendLine("            if (entity is null) return Results.Problem(statusCode: 404, title: \"Not Found\");");
            if (concurrency)
        ...
        if ((info.Operations & OpBulkCreate) != 0 && info.CreateRequestName is not null)
        {
            var fqCreate = info.Namespace is not null ? $"{info.Namespace}.{info.CreateRequestName}" : info.CreateRequestName;
            var validateMethod = info.IsCombinedDto ? "ValidateForCreate" : "Validate";
            sb.AppendLine($"        group.MapPost(\"bulk\", async (List<{fqCreate}> requests, {storeType} store{userParam}, CancellationToken ct) =>");
            sb.AppendLine("        {");
