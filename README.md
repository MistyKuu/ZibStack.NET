...
| [**ZibStack.NET.Aop**](packages/ZibStack.NET.Aop/) | `dotnet add package ZibStack.NET.Aop` | AOP framework with C# interceptors. Built-in: `[Retry]`, `[Cache]`, `[RequirePermission]`, `[Trace]`, `[Timing]`. Custom aspects via `IAspectHandler`/`IAroundAspectHandler`. |
