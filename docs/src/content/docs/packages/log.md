...
Compile-time logging for .NET 8+ using **C# interceptors**. Two features:

1. **`[Log]` attribute** — structured entry/exit/error logging on methods. Generated by the `ZibStack.NET.Aop` package (install it for `[Log]` to work). Supports `Apply<LogAttribute>()` for global registration without attributes.
2. **Interpolated string logging** — `_logger.LogInformation($"User {user}")` becomes structured and zero-allocation at compile time. This feature works from the `ZibStack.NET.Log` package alone.
...
```
dotnet add package ZibStack.NET.Aop    # handles [Log] generation + Apply<LogAttribute>
dotnet add package ZibStack.NET.Log    # interpolated-string logging (optional, independent)
```
